            ...
            <div class="col-11">
              <select class="form-control" id="barang_id" name="barang_id" required>
                ...
                @foreach($products as $product)
                  <option value="{{ $product->barang_id }}" @if (old('barang_id', $stok->barang->barang_id) === $product->barang_id) selected=true @endif >{{ $product->barang_nama }}</option>
                @endforeach
              </select>
              @error('barang_id')
            ...
            <div class="col-11">
              <select class="form-control" id="user_id" name="user_id" required>
                ...
                @foreach($users as $user)
                  <option value="{{ $user->user_id }}" @if (old('user_id', $stok->user->user_id) === $user->user_id) selected=true @endif >{{ $user->nama }}</option>
                @endforeach
              </select>
              @error('user_id')
              <small class="form-text text-danger">{{ $message }}</small>
